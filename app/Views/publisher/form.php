						...
						<div class="x_content">
							<form action="<?=base_url($target_url)?>" method="post" enctype="multipart/form-data">
								...
								<div class=" field item form-group">
									...
									<div class="input-group col-md-6 col-sm-6">
										<input type="num" name="contact" class="form-control <?=(!empty(session()->getFlashdata('validation')['contact'])) ? 'is-invalid' : ''?>" placeholder="" value="<?= ($is_edit) ? $item ['contact'] : old('contact') ?>">
										<div class="invalid-feedback">
